...
public partial class TreningVozac : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                command.CommandText = "SELECT * FROM personal_tip order by id";

                OracleDataAdapter adapter = new OracleDataAdapter();
                adapter.SelectCommand = command;
                DataSet data = new DataSet();
                adapter.Fill(data, "personal_tip");
    ...
    protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        try
        {
            OracleCommand command = new OracleCommand();
            command.Connection = connect;
            command.CommandText = "DELETE FROM personal_tip WHERE id = :id";
    ...
    protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        try
        {
            OracleCommand command = new OracleCommand();
            command.Connection = connect;
            command.CommandText = "UPDATE personal_tip SET ime=:ime, cena=:cena, trening_cena=:trening_cena, vkupna_jacina=:vkupna_jacina, trening_vrednost=:trening_vrednost WHERE ID=:ID";
            ...
            command.Parameters.AddWithValue(":trening_vrednost", Int32.Parse(tb.Text));
            ...
            if(effect != 0)
            {
                    command = new OracleCommand();
                    command.Connection = connect;
                    command.CommandText = "SELECT * FROM personal_tip";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            int vkupna_jacina = Int32.Parse(String.IsNullOrEmpty(TextBox5.Text.Trim()) ? "0" : TextBox5.Text.Trim());
            double trening_vrednost = Double.Parse(String.IsNullOrEmpty(TextBox6.Text.Trim()) ? "0" : TextBox6.Text.Trim());


            OracleCommand command = new OracleCommand();
            command.Connection = connect;
            command.CommandText = "INSERT INTO PERSONAL_TIP VALUES(:id, :ime, :cena, :trening_cena, :trening_vrednost, :vkupna_jacina)";
            command.Parameters.AddWithValue(":id", id);
            command.Parameters.AddWithValue(":ime", ime);
            command.Parameters.AddWithValue(":cena", cena);
            command.Parameters.AddWithValue(":trening_cena", trening_cena);
            command.Parameters.AddWithValue(":trening_vrednost", trening_vrednost);
